...
### 它能做什么
- 输入一整部影片（或一批影片），输出约 20–30 分钟、信息密度高、带故事脉络片头与解说转场的精华版；
- 自动交付 **B 站三件套**：成片 `_精华版.mp4`、封面 `cover.png`、文案 `B站三件套.txt`；
- 配套 **19 个 Python 脚本**，覆盖摸底 → 切点吸附 → 切香肠式分段截取 → edge-tts 解说 → 转场 → 拼接 → 音画同步质检 → 水印 → 交付自检全流程；
- **v13.5** 含双模型分工批处理 handoff 协议：强模型（max）做判断段落并落盘检查点，弱模型（flash）开新会话纯机械执行到交付，失败隔离、断点可续、成本更低。
...
```
film-highlight-editor/
├── SKILL.md              # 技能定义（v13.5，WorkBuddy agent 技能，含完整工作流与已知坑）
├── README.md             # 本文件（中英文双语）
├── LICENSE               # MIT © 上海翔远工作室 · MilesYang
├── CHANGELOG.md          # 版本记录
├── .gitignore
├── .env.example          # 交付配置模板（仅占位，不含任何密钥）
├── scripts/              # 19 个配套脚本（详见 SKILL.md「配套脚本」表）
...
- Python 依赖：`Pillow`、`numpy`、`edge-tts`、`rapidocr_onnxruntime`、`vosk`。
...
pip install Pillow numpy edge-tts rapidocr_onnxruntime vosk
```

### 快速开始
1. 在 `<WORK_ROOT>/`（你的工作区根目录）下按片名建文件夹，内建 `source/`（原片）、`work/`（中间产物）、`output/`（产出物）。
2. 原片放入 `source/`，运行 `scripts/contact_sheet.py` 摸底抽帧。
...
### What it does
- Takes a full movie (or a batch of movies) and produces a ~20–30 min, high-density highlight cut with a
  story-prologue and narration transitions.
- Auto-delivers the **Bilibili three-piece set**: final video `_精华版.mp4`, cover `cover.png`, and copy `B站三件套.txt`.
- Ships **19 Python scripts** covering the whole chain: scouting → cut snapping → sausage-style segment
  cutting → edge-tts narration → transitions → assembly → A/V sync QA → watermark → delivery self-check.
- **v13.5** adds a two-model handoff protocol: a strong model (max) judges segments and writes checkpoints;
  a weak model (flash) opens a fresh session and runs the mechanical chain to delivery, with failure isolation,
  resumability, and lower cost.
...
### Repository layout
See the tree above. `SKILL.md` is the authoritative workflow + known-pitfalls doc; `scripts/` holds the 19 tools.

### Requirements
- **ffmpeg** ≥ 4.x (libx264 / AAC; ffprobe for QA).
- **Python** ≥ 3.11 in an isolated venv.
- Python deps: `Pillow`, `numpy`, `edge-tts`, `rapidocr_onnxruntime`, `vosk`.
...
### Quick start
1. Under `<WORK_ROOT>/` create a per-film folder with `source/` (original), `work/` (intermediates), `output/` (deliverables).
2. Put the original in `source/`, run `scripts/contact_sheet.py` to scout frames.
